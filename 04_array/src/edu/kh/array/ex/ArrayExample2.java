...
public class ArrayExample2 {
	...
	*/
	public void deepCopy() {
		...
		//1. for문을 이용한 깊은 복사
		int[] copyArr1 = new int[arr.length]; //5칸 짜리 배열 new int[5];
		...
		System.arraycopy(arr, 0, copyArr2, 0, arr.length); //마지막에 칸의 수라서 5 
	...
	public void createLottoNumber() {
		...
		for(int i = 0; i < lotto.length; i++) {
			...
			//3) 1 ~ 45 사이에 난수 생성
			int random = (int)(Math.random() * 45 + 1 ); // 1 ~ 45 // 1 <= x < 46
			...
			for(int x = 0; x < i; x++) {
				...
				if(random == lotto[x]) {
					...
					//-> i값을 인위적으로 1 감소시켜서 난수 발생의 기회를 한 번더 주는 것.
					break; // 앞쩍에서 중복데이터를 발견하면 남은 값은 비교할 필요X 
		...
		//7) 오름차순 정렬 > 선택, 삽입, 버블, 퀵 등등
		// 자바가 정렬 방법을 미리 만들어서 제공 하고 있음
