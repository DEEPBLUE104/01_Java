...
public class StudentService {
	...
	// new ArrayList<>() : 기본 생성자 -> 기본 크기 10짜리의 리스트 생성
	// 하지만 리스트의 크기는 늘었다 줄었다 하기에 큰 의미는 없다
	...
	public StudentService() {
		studentList.add(new Student("서정욱", 18,"경기도 양주", 'M' , 28));
	...
	public void displayMenu() {
		...
		do {
			...
			System.out.println("7. 나이순으로 정렬"); // comparable 사
	 ...
	 * 
	 * @return
	 *  삽입 성공 시 "성공" , 실패 시 "실패" 문자열 반환
	 */
	public String addStudent() { 
		// checked Exception : 반드시 throw 구문이나 try-catch 처리 강제
		// IOException, SQLException..
		
		// Unchecked Exception : 강제 처리 X, 예외 발생하면 자동으로
		// 상위 호출 메서드로 예외가 전파됨.
		//InputMismatchException, NullPointerException 등등
	...
	public void selectAll() {
		...
		// 일반 for 문
		/*
		for (int i = 0; i < studentlist.size(); i++) { //length(배열때만!) 
	...
	public void searchName1() {
		...
		for(Student std : studentList) {
			if(input.equals(std.getName())){ // 이름이 일치하는 경우
				...
				flag = false; //찾았다면 flag로 중지
			}
		}
		
		if(flag) { //학생의 이름을 못 찾은 경우
	 ...
	 * 객체로 생성하면서 바로 구현 내용을 정의할 수 있음
	 * - 익명 내부 클래스 장점
	 * 1. 코드 간결화 (별도로 클래스 만들지 않아도 될 때 사용)
	 * 2. 즉시 사용가능 (한 번만 사용할 Comparator 등을 정의할 때 유용함)
