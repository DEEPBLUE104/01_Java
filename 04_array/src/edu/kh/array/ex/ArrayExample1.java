...
public class ArrayExample1 {
	...
	public void ex1() {
		...
		System.out.println("arr :" + arr); //주소 이름
	...
	//arr[0~3] 전부JVM에 의해 0으로 초기화 되어있는 상태
		...
		/*Exception in thread "main" java.lang.ArrayIndexOutOfBoundsException: Index 4 out of bounds for length 4
			배열의 인덱스 범위를 벗어남 : 길이가 4칸짜리 배열인데, 없는 인덱스 번호에 접근함. */
		
		// 코드에서 빨간줄 에러 : 컴파일 에러
		// 실행했을 때 발생하는 에러 : 런타임 에러
	
		//배열과 for문
		//배열의 길이 (몇 칸짜리인가.) 배열명.length
	...
	public void ex4() {
		...
		// =======================
		// > score 배열의 각 요소가 모두 입력된 값으로 대입 완료
	...
	public void ex6() {
		...
		//그러나 flag가 false라면 if조건식이 false이므로 수행되지 않음
		// > not 연산자 : 부정 논리 연산자 (!)를 붙여줌으로서 true로 변경하여
		//{} 안에 있는 코드를 수행하게끔 함.
	...
	public void ex7() {
		...
		for(int i = 0; i < arr.length; i++) {
			if (arr[i].equals(fruit)) { //String 비교시 equals()사용
				System.out.println(i + "번째 인덱스 존재");
	...
	public void ex8() {
		...
		String str = "가나디"; //String Constant pool
		String str2 = new String("가나디");   //Heap 새 객체 저앙
		System.out.println(str); //주소값
		System.out.println(str2); //주소값
		...
		System.out.println(System.identityHashCode(str)); //String 메서드 이름 정라기
	...
	public void ex9() {
		...
		for(int i = 0; i < arr.length; i++) {
			if(arr[i] == ch) {
				count++; // arr[i] 값과 검색할 문자가 ch가 같은 경우
						 // 카운트를 늘려라
			}
		}
