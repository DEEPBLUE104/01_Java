...
public class ObjectService {
	...
	public void ObjectOutput() {
		...
		try {
			...
			oos = new ObjectOutputStream(fos);  //보조스트림이니 기반 스트림을 대입
												//객체 보조 스트림 생성					
			// Member 객체 하나 생성
			Member member = new Member("member01","pass01","김회원",30);
			
			//ObjectOutputStream 이용해서 Member 객체를 출력하기
			oos.writeObject(member);  //member 객체를 연결된 경로의 파일로 쓰겠다.(출력)
			// java.io.NotSerializableException
			// edu.kh.io.pack3.model.dto.Member
			// -> 직렬화 되지 않음
			// -> Member 클래스를 직렬화하기 위해 implements Serializable 해야함
	...
	public void ObjectInput() {
		...
		try {
			...
			//Objext.ois.readObject(); : 직렬화된 객체를 읽어와 역직렬
			//							+ Member 객체를 다운캐스팅
